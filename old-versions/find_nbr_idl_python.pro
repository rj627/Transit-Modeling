pro find_nbr_idl_python, fpath, apr, a, b, c, sm_num

;def nnbr_from_idl(path, apr, sm_num , a , b , c):
;    import pidly
;    from numpy import genfromtxt
;    idl=pidly.IDL('/Applications/exelis/idl85/bin/idl')
;    idl.pro('find_nbr_idl_python', path, str(apr), a, b, c, sm_num)
;    gw = genfromtxt(path+'gw_out_'+str(apr), delimiter=',')
;    nbr_ind = genfromtxt(path+'nnbr_out_'+str(apr), delimiter=',')
;    print (nbr_ind.shape, gw.shape)
;
;    return np.transpose(nbr_ind), np.transpose(gw)


data=read_csv(fpath+'pos_out_'+apr)
xpos=data.Field1
ypos=data.Field2
np=data.Field3
...
for point=0L,n-1 do begin
  ...
  print,fpath+'nnbr_out_'+apr
  WRITE_CSV, fpath+'nnbr_out_'+apr, nbr_ind
  WRITE_CSV,fpath+'gw_out_'+apr, gw
